<template>
  <div class="questionAddView">
    <h1 style="text-align: center">修改题目</h1>
    <a-form
      ref="formRef"
      :rules="rules"
      :size="formSize"
...
import { QuestionAdminDto, Service } from '../../../openapi';

const tagNameList = ref<Array<string>>([]);

const formSize = ref('medium');
const form = reactive<QuestionAdminDto>({
  title: '',
  description: '',
  tagList: [],
  difficult: 0,
...
onMounted(async () => {
  ...
  const res = await Service.adminGetById(questionId);
  if (res.code === 0) {
    Object.assign(form, res.data);
    console.log('form:', form.judgeCase);
  }
  const tagRes = await Service.getTagNameList();
  if (tagRes.code === 0) {
    tagNameList.value = tagRes.data ?? [];
  }
});
...
const submit = async () => {
  console.log(form);
  const res = await Service.updateQuestion(form);
  if (res.code === 0) {
    respSuccess('修改成功');
  } else {
